...
function stubMatchMedia(window, options) {
  ...
  window.matchMedia = function (query) {
    var matches = false;
    if (String(query).indexOf('max-width: 780px') !== -1) matches = !!opts.mobile;
    else if (String(query).indexOf('hover: none') !== -1) matches = !!opts.hoverNone;
    else if (String(query).indexOf('prefers-reduced-motion') !== -1) matches = opts.reducedMotion !== false;
    return {
      matches: matches,
      media: query,
